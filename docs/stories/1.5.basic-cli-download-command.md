...
- [ ] Create ProgressTracker service interface with state management
- [ ] Implement console-based progress display using ANSI escape codes
- [ ] Track metrics: total/processed expirations, records count, elapsed time
- [ ] Display real-time updates: [current/total] expirations, completion %, records/sec
- [ ] Use process.stdout.write with '\r' for in-place terminal updates
- [ ] Calculate and store summary statistics during processing

### Task 5: Summary Statistics Implementation (AC: 5)
- [ ] Collect metrics during processing: per-file record counts, file sizes, timestamps
- [ ] Track: total/successful/failed expirations, total records, total bytes, processing time
- [ ] Display formatted summary with sections: header, summary stats, file list
- [ ] Format numbers: use commas (45,678), file sizes (23.4 MB), duration (2m 15s)
- [ ] Show throughput as records/sec, list top files with size and record count
- [ ] Use box-drawing characters for professional CLI output (═, ─, etc.)

### Task 6: Dry Run Mode Implementation (AC: 6)
- [ ] Add --dry-run flag option to download command using Options.boolean
- [ ] When dry-run enabled, fetch expiration list but skip actual data download
- [ ] Display what would be downloaded: number of expirations, estimated size
- [ ] Show directory structure that would be created
- [ ] Return without making actual API calls for bulk data
...
- [ ] Special handling for Terminal not running with helpful message
...
- [ ] Add dry-run option using `Options.boolean` with default false

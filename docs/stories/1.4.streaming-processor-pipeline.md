...
[ ] 7. Provide real-time progress updates with throughput metrics (records/sec, completion %, memory usage). Persist ALL run metrics using MetricsWriter service for historical analysis
[ ] 8. Support graceful interruption with proper resource cleanup
[ ] 9. Ensure atomic file writes to prevent corruption
[ ] 10. Create MetricsWriter abstraction with JsonMetricsWriter implementation that stores metrics in `./data/metrics/` with no expiration (keep all history)
...
- [ ] Define MetricsWriter service interface in `src/services/MetricsWriter.ts`
- [ ] Create Context.Tag following Effect-TS patterns
- [ ] Define PipelineRunMetrics type with comprehensive metrics
- [ ] Define MetricsQuery type for future querying capability
- [ ] Create MetricsWriterError tagged error type
...
- [ ] Create JsonMetricsWriter in `src/layers/JsonMetricsWriter.ts`
- [ ] Implement writeMetrics to save JSON files with timestamp naming
- [ ] Implement readMetrics with optional filtering
- [ ] Ensure metrics directory creation on first write
- [ ] Add file rotation logic if needed for performance
...
- [ ] Write tests in `test/services/MetricsWriter.test.ts`
...
- [ ] Test MetricsWriter persistence and retrieval

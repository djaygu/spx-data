...
// E2E tests that download real data from ThetaData Terminal
...
describe.skipIf(!SHOULD_RUN_E2E_TESTS)("End-to-End Download Test", () => {
  ...
  beforeAll(async () => {
    ...
    const tempBase = `/tmp/spx-data-e2e-${Date.now()}-${Math.random().toString(36).slice(2)}`
  ...
  it(
    ...
    async () => {
      ...
      const testDate = "2025-08-06" // A Monday in January 2024
      ...
      expect(await dirFile.exists()).toBe(true)
  ...
  it(
    ...
    async () => {
      ...
      const outputDir = path.join(tempDir, "20240115-dry")
  ...
  it(
    ...
    async () => {
      const testDate = "2024-01-15"
      const outputDir = path.join(tempDir, "20240115-dte")

      // Set environment variables for configuration
      process.env.CONFIG_STORAGE_DATA_DIRECTORY = path.join(tempDir, "-dte")
      process.env.CONFIG_DOWNLOAD_CONCURRENCY = "2"
      ...
      const tradeDate = new Date(2024, 0, 15) // January 15, 2024
  ...
  it(
    ...
    async () => {
      const testDate = "2024-01-15"
      const outputDir = path.join(tempDir, "20240115-validate")

      // Set environment variables for configuration
      process.env.CONFIG_STORAGE_DATA_DIRECTORY = path.join(tempDir, "-validate")
      process.env.CONFIG_DOWNLOAD_CONCURRENCY = "2"
      process.env.CONFIG_DOWNLOAD_GREEKS_BATCH_SIZE = "100"
      process.env.CONFIG_DOWNLOAD_CSV_BATCH_SIZE = "1000"
